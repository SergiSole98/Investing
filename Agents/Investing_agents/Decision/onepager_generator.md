...
2. Read the following ten files:
...
2. If any of the ten source files is missing, list which ones are absent and stop; do not produce a partial one-pager.
...
5. HECHOS BASE must cite only sources already present in `news.md`, `news_scored.md`, and `positions.md`; do not add new citations. Include intentionality flags from `news_scored.md` where relevant.
