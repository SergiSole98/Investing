...
3. For each news item and each event, evaluate three dimensions:
   ...
   - **Intentionality flag:** flag the item if the publishing or quoted entity has a **disclosed financial interest** in the asset — e.g., an analyst with a public price target, an institution with a stated position, or the company itself issuing a press release. Note what the entity gains from this narrative being believed.
...
- The intentionality check is factual, not an accusation; flag the disclosed interest without editorializing.

## Rules

1. If any of the two source files is missing, list which ones are absent and stop; do not produce a partial output.
...
4. Intentionality flag must only be applied when the publishing or quoted entity has a verifiable, publicly disclosed financial interest in the asset. Do not flag based on sector or category alone.
...
- **`Agents/Investing_agents/orquestador.md`** — Invokes this agent after news_researcher and event_scanner confirm.
